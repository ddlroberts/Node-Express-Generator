  ...
  .put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        ...
        if (campsite && currentComment) {
          //check if login user is author of comments
          let loggedInUserId = req.user._id;
          //console.log(loggedInUserId);
          let currentCommentAuthor = currentComment.author;
          //console.log(currentCommentAuthor);
          if (currentCommentAuthor.equals(loggedInUserId)) {
            if (req.body.rating) {
            ...
            err = new Error("You are not authorized to add this comment");
            err.statusCode = 403;
            return next(err);
          }
        } else if (!campsite) {
